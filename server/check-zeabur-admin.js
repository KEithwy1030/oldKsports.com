...
const checkZeaburAdmin = async () => {
    try {
        console.log('检查Zeabur数据库中的管理员账号...');
        ...
        const usersByUsername = await new Promise((resolve, reject) => {
            db.query('SELECT id, username, email, is_admin, points, created_at FROM users WHERE username = ?', ['老k'], (err, result) => {
                if (err) reject(err);
                else resolve(result);
            });
        });
        ...
        console.error('检查Zeabur数据库失败:', error);
    } finally {
        db.end();
    }
};
